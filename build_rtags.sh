...
if [ $OS = "Ubuntu" ]; then
    ...
    if [ "$CLANGVER" -lt 11 ]; then
        echo "clang++ must be >= 11, current: $CLANGVER"
        exit 1
    fi
    if [ ! -d /opt/rh/llvm-toolset-11.0 ]; then
        echo "yum install llvm-toolset-11.0 llvm-toolset-11.0-clang-devel llvm-toolset-11.0-llvm-devel devtoolset-11"
        echo "scl enable devtoolset-9 llvm-toolset-11.0"
        exit 1
    fi
    ...
    /usr/bin/cmake3 .. $COMMON -DLLVM_CONFIG=/opt/rh/llvm-toolset-11.0/root/bin/llvm-config
else
